...
def fetch_latest_event_by_name(db_path: Path, limit: int=1, event_name: str="Highest Temperature")->List[Dict]:
    ...
    try:
        cur=cursor.cursor()
		cur.execute("""SELECT EventTypes.Name, EventTypes.Description, SensorRecords.*  FROM SensorEvents INNER JOIN EventTypes ON SensorEvents.TypeID=EventTypes.TypeID INNER JOIN SensorRecords ON SensorEvents.RecordID=SensorRecords.RecordID WHERE EventTypes.name=? ORDER BY DateTime DESC LIMIT ?
			""", (event_name,limit),
        )
        rows=cur.fetchall()
        return [dict(row) for row in rows]
